// Archived corrupted seeder file
// The original file contained concatenated fragments and caused syntax errors.
// A cleaned canonical seeder now exists at `seedDatabase.js`.
// This file is intentionally left as a harmless placeholder to avoid
// accidental execution. Kept in the repo for forensic reference.

module.exports = {};
#!/usr/bin/env node
/**
 * seedDatabase.js
 * Clean, single seeder file. Uses two modes:
 *  - JSON-driven: consumes backend/seedData/*.json (companies, jobs, profiles)
 *  - Programmatic: small deterministic fallback seeder
 *
 * Usage:
...
async function connect() {
  // Let mongoose use defaults (avoid deprecated driver options)
...
async function clearAll() {
  ...
  await Company.deleteMany({}).catch(() => {});
  await UserProfile.deleteMany({}).catch(() => {});
  await User.deleteMany({}).catch(() => {});
}

function readSeedData(filename) {
  const p = path.join(__dirname, 'seedData', filename);
  if (!fs.existsSync(p)) return null;
  try {
    return JSON.parse(fs.readFileSync(p, 'utf8'));
  } catch (e) {
    console.warn('Failed to parse seedData file', p, e.message);
    return null;
  }
}

async function createOrFindUserByEmail(email, role = 'jobseeker') {
...
async function seedProgrammatic() {
  console.log('Running deterministic programmatic seeder...');

  const employer = await createOrFindUserByEmail('employer+seed@example.com', 'employer');
  const jobseeker = await createOrFindUserByEmail('jobseeker+seed@example.com', 'jobseeker');

  let company = await Company.findOne({ name: 'Seed Company Ltd' });
  if (!company) {
  ...
  try { employer.company = company._id; await employer.save(); } catch (e) {}
