...
/**
 * seedDatabase.js
 * Robust seeder that supports two modes:
 *  - If `backend/seedData/{companies,jobs,profiles}.json` exist, it will consume them and create records.
 *  - Otherwise it falls back to a small deterministic programmatic seeder.
 *
 * Usage:
 *   node seedDatabase.js           # seed (append)
 *   node seedDatabase.js --clear   # clear then seed
 */
...
async function connect() {
  // Avoid passing deprecated driver options
...
async function seedProgrammatic() {
  console.log('Running small deterministic seeder...');
